...
@Slf4j
@Component
public class GeneratorTaskNameUtil {

    /**
     * 生成标题和描述的提示词
     */
    private static final String titleGeneratorPrompt = """
        请从以下描述中提取核心信息，生成一个准确概括内容的标题和内容描述：
        {{content}}
        ...
        3. 描述长度控制在50-100字之间
        4. 必须以JSON格式返回结果，仅包含"title"和"desc"字段
        5. 确保JSON格式严格正确，无任何多余字符或注释
    ...
    @Autowired
    private IDeepSearchTaskService deepSearchTaskService;
    ...
    public void generatorDeepSearchDocumentName(DeepSearchTaskEntity taskEntity, String promptText) {
        // 记录任务开始日志
     ...
     * 生成并更新标题，包含重试机制
     */
    private void generateAndUpdateTitle(DeepSearchTaskEntity taskEntity, String promptText, String modelId) {
        int retryCount = 0;
     ...
     * 处理生成失败的情况
     */
    private void handleGenerationFailure(DeepSearchTaskEntity taskEntity) {
        // 可以设置默认标题或更新任务状态
