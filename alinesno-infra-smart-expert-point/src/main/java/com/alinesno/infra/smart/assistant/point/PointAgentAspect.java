...
//
//            // 初始化处理器
//            AgentPointProcessor agentProcessor = new AgentPointProcessor(method , agentPointAnnotation);
//
//            // 检查限流
//            if (!agentProcessor.checkRateLimit(userId, orgId)) {
//                throw new RpcServiceRuntimeException("请求频率超过限制");
//            }
//
//            // 2. 执行目标方法并统计耗时
//            try {
//                agentProcessor.setRateLimit(orgId , userId) ;
//
//                Object result = joinPoint.proceed(); // 执行实际方法
//
//                // 处理异步方法的情况
//                if (result instanceof DeferredResult<?> deferredResult) {
//                    deferredResult.onCompletion(() -> {
//                        long cost = System.currentTimeMillis() - startTime;
//                        log.info("异步方法 {} 执行完成，总耗时 {} ms", method.getName(), cost);
//
//                        agentProcessor.deductPoints(userId, orgId);
//                        agentProcessor.process(request, userId, orgId);
//                    });
//
//                    deferredResult.onError(throwable -> {
//                        long cost = System.currentTimeMillis() - startTime;
//                        log.error("异步方法 {} 执行失败，耗时 {} ms", method.getName(), cost, throwable);
//                        agentProcessor.process(request, userId, orgId);
//                    });
//                } else {
//                    // 同步方法的处理
...
//            } catch (Exception e) {
//                long cost = System.currentTimeMillis() - startTime;
//                log.error("方法 {} 执行失败，耗时 {} ms", method.getName(), cost, e);
//                agentProcessor.process(request, userId, orgId);
//                throw e;
//            }
//        }
