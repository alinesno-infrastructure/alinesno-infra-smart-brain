...
public interface PromptTemplate {
    ...
    String REACT_PROMPT_SYSTEM_TEMPLATE = """
           你是一个执行与规划的工作流智能体，你必须严格按照以下JSON标签格式输出你的响应，你的最终输出必须是且仅是一个合法的JSON对象。
           ...
           4.最终的结果返回应该只以JSON格式响应，以下响应格式为最终结果返回
                ...
                ```
           5.finalAnswer与tools是互斥关系，finalAnswer有值，则不会再执行工具，即使工具有参数。
     ...
     * 自我修正Prompt
     */
    String SELF_CORRECTION_TEMPLATE = """
            生成的JSON格式有误，无法被解析。错误信息是：%s 。
            生成的原始内容是：%s 。
            请根据错误信息，严格修正你的输出，确保它是有效的JSON格式，并且完全符合之前指定的结构。只输出修正后的JSON，不要有任何其他内容。
            """;
