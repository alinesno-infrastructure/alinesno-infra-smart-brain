...
public class ExamPagerFormatMessageTool {
     ...
     */
    @Async
    public void markExamScore(ExamScoreEntity examScore, IndustryRoleEntity industryRole, ExamPagerSceneEntity examPagerScene, ExamInfoEntity examInfo) {

        log.info("开始阅卷，考试ID：{}", examScore.getExamInfoId());

        // 更新状态为阅卷中
        examScore.setExamStatus(ExamineeExamEnums.REVIEW.getCode());
        examScoreService.updateById(examScore);

        // 启动会话，试卷属于哪个组织，则从哪个组织扣分
        long accountId = examInfo.getOperatorId() ;
        long orgId = examInfo.getOrgId() ;
        accountPointService.startSession(accountId, orgId , industryRole.getId());
        ...
        roleService.runRoleAgent(taskInfo)
                .thenAccept(workflowExecutionDto -> {
                    ...
                    // 会话结束
                    accountPointService.endSession(accountId, orgId , industryRole.getId());
                })
                .exceptionally(ex -> {
