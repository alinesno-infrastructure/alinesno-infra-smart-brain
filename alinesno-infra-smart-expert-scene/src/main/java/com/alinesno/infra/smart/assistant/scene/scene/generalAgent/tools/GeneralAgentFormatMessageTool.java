...
@Slf4j
@Component
public class GeneralAgentFormatMessageTool {
    ...
    public void handleChapterMessage(WorkflowExecutionDto genContent, MessageTaskInfo taskInfo) {
        ...
        String formatContent = """
         ##角色：你是一个专业的解决思路规划，非常善于针对解决思路分析做规划，创作一个超长解决思路分析大纲，并且对解决思路分析大纲中
         ...
         ##规则：
          1-在创作解决思路分析大纲的时候，请你主要【附件资料】为主，调取你这个领域的专业数据进行创作大纲。
          2-在创作解决思路分析大纲的时候，请你平均拆分为N个部分，未来将有N个不同的智能体按照你的大纲进行创作具体的文字内容。请你保证N个部分边界清晰，没有重叠，避免冲突。
          3-在你创作的解决思路分析大纲的N个部分的时候，每个部分需要给出一个标题，然后给出一个具体的编写这部分内容编写要求，目的是给下一步拆分后的创作智能体参考。
          4-在你创作的N个部分编写要求中，除了对这部分的内容进行描述，也就是创作什么具体的内容。
          5-在编写要求中，请你根据用户的文字总数进行计算，对每一个部分创作具体内容时候要求的字数，给出要求，并且每个部分的内容不要少于500个字。
        ...
        AgentSceneEntity agentSceneEntity =  agentSceneService.getByRoleAndScene(taskInfo.getRoleId(), SceneEnum.GENERAL_AGENT.getSceneInfo().getId()) ;
