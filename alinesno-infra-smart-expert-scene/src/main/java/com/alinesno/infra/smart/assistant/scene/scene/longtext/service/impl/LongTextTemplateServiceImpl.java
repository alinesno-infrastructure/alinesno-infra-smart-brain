...
import com.alinesno.infra.smart.assistant.scene.scene.longtext.mapper.LongTextTemplateMapper;
...
@Slf4j
@Service
public class LongTextTemplateServiceImpl extends IBaseServiceImpl<LongTextTemplateEntity, LongTextTemplateMapper> implements ILongTextTemplateService {
    ...
    @Override
    public List<LongTextTemplateEntity> getTemplateByType(PermissionQuery query, String typeCode) {
        ...
        LambdaQueryWrapper<LongTextTemplateEntity> wrapper = new LambdaQueryWrapper<>();

        // 先构建 OR 条件（PUBLIC_SCENE 或 ORG_SCENE+orgId）
        wrapper.and(w -> w
                .eq(LongTextTemplateEntity::getLongTextTemplatePermission, SceneScopeType.PUBLIC_SCENE.getValue())
                .or()
                .eq(LongTextTemplateEntity::getLongTextTemplatePermission, SceneScopeType.ORG_SCENE.getValue())
                .eq(LongTextTemplateEntity::getOrgId, orgId)
        );

        // 再添加 typeCode 条件（如果有）
        if (typeCode != null && !typeCode.isEmpty()) {
            wrapper.eq(LongTextTemplateEntity::getLongTextTemplateType, typeCode);
        } else {
            wrapper.last("LIMIT 100");
        }


        wrapper.orderByDesc(LongTextTemplateEntity::getAddTime);
    ...
    @Override
    public Map<String, Object> readExcel(List<RobustExcelParser.TemplateBean> templates, PermissionQuery query) {
        // 初始化失败列表和成功列表
        List<LongTextExcelData> failedList = new ArrayList<>();
        List<LongTextTemplateEntity> successList = new ArrayList<>();
        // 用于保存分组和模板的映射关系
        Map<Long, List<LongTextTemplateEntity>> groupTemplateMap = new HashMap<>();

        try {
            // 获取分组后的模板数据
            List<RobustExcelParser.TemplateGroup> fullGroups = RobustExcelParser.getTemplateGroups(templates);

            // 遍历每个分组
            for (RobustExcelParser.TemplateGroup group : fullGroups) {
                try {
                    // 处理分组信息并保存到数据库
                    LongTextTemplateGroupEntity templateGroup = processTemplateGroup(group , query);

                    // 处理当前分组下的所有模板
                    List<LongTextTemplateEntity> templateEntities = processTemplates(group, templateGroup, failedList , query);
     ...
     * @return 保存后的分组实体
     */
    private LongTextTemplateGroupEntity processTemplateGroup(RobustExcelParser.TemplateGroup group, PermissionQuery query) {
        LongTextTemplateGroupEntity templateGroup = new LongTextTemplateGroupEntity();
        ...
        longTextTemplateGroupService.save(templateGroup); // 保存分组
     ...
     * @return 处理成功的模板实体列表
     */
    private List<LongTextTemplateEntity> processTemplates(RobustExcelParser.TemplateGroup group,
                                                          LongTextTemplateGroupEntity templateGroup,
                                                          List<LongTextExcelData> failedList,
                                                          PermissionQuery query) {
        List<LongTextTemplateEntity> templateEntityList = new ArrayList<>();

        // 遍历分组下的每个模板
        for (RobustExcelParser.TemplateBean template : group.getTemplates()) {
            try {
                // 创建模板实体
                LongTextTemplateEntity templateEntity = createTemplateEntity(template, templateGroup , query);
     ...
     * @return 模板实体
     */
    private LongTextTemplateEntity createTemplateEntity(RobustExcelParser.TemplateBean template,
                                                        LongTextTemplateGroupEntity templateGroup, PermissionQuery query) {
        LongTextTemplateEntity templateEntity = new LongTextTemplateEntity();
     ...
     */
    private void addGroupToFailedList(RobustExcelParser.TemplateGroup group,
                                      List<LongTextExcelData> failedList,
                                      String errorMessage) {
        // 将该分组下所有模板添加到失败列表
        for (RobustExcelParser.TemplateBean template : group.getTemplates()) {
     ...
     * @return 失败记录对象
     */
    private LongTextExcelData createFailedRecord(RobustExcelParser.TemplateBean template, String errorMessage) {
    ...
    private boolean isExistByName(String title, PermissionQuery query) {
        return this.lambdaQuery()
                .eq(LongTextTemplateEntity::getName, title)
                .eq(LongTextTemplateEntity::getOrgId, query.getOrgId())
                .exists();
    }
