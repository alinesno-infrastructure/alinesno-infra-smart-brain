    ...
    public void handleChapterMessage(WorkflowExecutionDto genContent, MessageTaskInfo taskInfo, LongTextTemplateEntity longTextTemplate) {
        ...
        String formatContent = """
         ...
         ##规则：
          ...
          6-在编写要求时，如果是非叶子节点，不能与叶子节点要求重复，则要求对叶子节点进行总结，避免与叶子节点内容重复，不能与叶子节点的内容进行重复。
          7-在编写要求中，请你根据用户的文字总数进行计算，对每一个部分叶子节点创作具体内容时候要求的字数，给出要求。如果用户没有给出字数要求，请你按照每一叶子节点标题内容要求在1000字以上。
          8-最终请你下面的格式进行直接输出，不要输出其他信息，格式：
           ```json
          [
              {
                  "label": "第一部分大纲标题",
                  "description": "第一部分编写要求，如果是非叶子节点，则要求对叶子节点进行总结，避免与叶子节点内容重复",
                  "children": [
                      {
                          "label": "第一部分大纲子标题1",
                          "description": "第一部分大纲子标题1编写要求，如果是非叶子节点，则要求对叶子节点进行总结，避免与叶子节点内容重复"
                          "children": [
                              {
                                  "label": "第一部分大纲子标题1子标题1",
                                  "description": "第一部分大纲子标题1子标题1编写要求，如果是非叶子节点，则要求对叶子节点进行总结，避免与叶子节点内容重复"
                                  "children": [
                                      {
       ...
       String templatePromptContent = StringUtils.EMPTY;
       if(longTextTemplate != null) {
          templatePromptContent = String.format(
                   templatePrompt ,
