...
import com.alinesno.infra.smart.assistant.screen.agent.event.ToolExecuteEvent;
...
@Slf4j
@Component
public class ToolExecuteListener {
    ...
    public ToolExecuteListener(ApplicationEventPublisher publisher) {
        this.publisher = publisher;
    }

    @SneakyThrows
    @EventListener
    public void execute(ToolExecuteEvent event) {

        RoleTaskDto task = event.isLeader()? event.getLeaderTask(): event.getWorkerTask() ;
        log.debug("Tool execute: name = {} , params = {} "  , task.getToolName(), task.getToolParams());
        ...
        task.setKnowledgeContent(content);

        if(event.isLeader()){
            publisher.publishEvent(new TaskCompletionEvent(this, event.getWorkerTask() , task, false));
        }else{
