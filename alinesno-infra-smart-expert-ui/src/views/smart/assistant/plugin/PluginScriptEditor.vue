<template>
  <div class="tool-cm-container">
    <code-mirror basic :lang="lang" v-model="codeVal" :style="'height:calc(100vh - 195px)'" :theme="theme"
    ...
    <el-dialog v-model="dialogVisible" :title="'函数内容（Groovy）'" append-to-body fullscreen>
        <template #header>
          <div class="dialog-footer mt-24" style="display: flex;justify-content: space-between;align-items:center">
            ...
            <div>
              <!-- <el-button type="warning" @click="validateScript" bg text size="large"> 验证任务</el-button> -->
              <el-button type="primary" @click="submitDialog" bg text size="large"> 确认 </el-button>
            </div>
          </div>
        </template>
...
.tool-cm-container .function-CodemirrorEditor__footer {
  position: relative;
  bottom: 40px;
  right: 10px;
  float: right;
