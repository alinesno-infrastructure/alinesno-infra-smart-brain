<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="open" width="900px" append-to-body>
      <el-form :model="form" :rules="rules" ref="PluginRef" label-width="80px">
        <el-row>
          <el-col :span="24">
            <el-form-item  label="插件名称" prop="name">
...
import {
  ...
  updatePlugin,
  addPlugin,
} from "@/api/smart/assistant/plugin";
import {reactive} from "vue";

const router = useRouter();
const {proxy} = getCurrentInstance();
// const { sys_normal_disable, sys_Plugin_sex } = proxy.useDict("sys_normal_disable", "sys_Plugin_sex");
...
const upload = reactive({
  ...
  // 上传的地址
  url: import.meta.env.VITE_APP_BASE_API + "/system/Plugin/importData"
});
// 列显隐信息
const columns = ref([
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除应用编号为"' + applicationIds + '"的数据项？').then(function () {
    return delPlugin(applicationIds);
  }).then(() => {
    getList();
...
function handleUpdate(row) {
  ...
  getPlugin(applicationId).then(response => {
