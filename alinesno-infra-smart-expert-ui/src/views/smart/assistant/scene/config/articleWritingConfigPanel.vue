        ...
        <el-row :gutter="20">
            ...
            <el-col :span="5" :xs="24">
                <div class="plugin-catalog">
                    ...
                    <div class="catalog-content">
                      <div  
                        class="catalog-item" 
                        :class="{ 'active': currentArticleTemplateType === '' }"
                        ...
                        <div 
                            ...
                            class="catalog-item" 
                            :class="{ active: category.code === currentArticleTemplateType }">
                            ...
                            <!-- 
                            <span v-if="getCategorySelectedCount(category.code) > 0" class="selected-count">
                                {{ getCategorySelectedCount(category.code) }}
                            </span> 
                            -->
            ...
            <el-col :span="19" :xs="24">
                ...
                <div v-loading="loading" class="template-card-list">
                    <div 
                        v-for="(template, index) in ArticleTemplateList" 
                        :key="index" 
                        class="template-card"
                    ...
                    >
                        <div class="card-header">
                            ...
                            <div class="card-select-indicator">
                                <!-- <el-checkbox 
                                    :label="template.id"
                                    @change="(checked) => handleCheckboxChange(checked, template)"
...
import { listArticleTemplate, getAllArticleTemplate } from "@/api/smart/scene/articleTemplate";
...
// 处理复选框变化
const handleCheckboxChange = (checked, template) => {
    if (checked && !selectedTemplates.value.some(item => item.id === template.id)) {
        selectedTemplates.value.push({...template});
    } else if (!checked) {
        selectedTemplates.value = selectedTemplates.value.filter(item => item.id !== template.id);
    }
...
const getCategorySelectedCount = (categoryCode) => {
    // 这里假设模板中有一个category字段与分类的code对应
    // 如果实际字段名不同，请根据实际情况修改
    return selectedTemplates.value.filter(
        item => item.articleTemplateType === categoryCode
    ).length;
};

// 获取分类列表
const getArticleTemplateTypeList = async() => {
    await getAllArticleTemplate().then(response => {
...
onMounted(async () => {
    ...
    // 关键：初始化后加载模板列表，确保模板的 articleTemplateType 字段存在
    getList(); 
    
    // 获取分类列表（原逻辑保留）
    await getArticleTemplateTypeList();
...
const reCountPluginCategories =() => {
    if(pluginCategories.value){
      ...
      for(var i = 0 ; i < pluginCategories.value.length ; i ++){
        ...
        const categorySelectedCount = getCategorySelectedCount(item.code);

        console.log("item id = " + item.code + " categorySelectedCount = " + categorySelectedCount);
...
const getConfigData = () => {
  ...
  const simplifiedTemplates = selectedTemplates.value.map(template => ({
      ...
      articleTemplateType: template.articleTemplateType,        // 所属类型 
