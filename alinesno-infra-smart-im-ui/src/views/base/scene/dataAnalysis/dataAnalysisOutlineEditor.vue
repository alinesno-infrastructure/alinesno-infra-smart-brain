            ...
            <template #header>
                <div class="card-header">
                    <div style="display: flex;align-items: center;gap: 5px;">
                        ...
                        <el-tooltip v-for="(item,index) in currentSceneInfo.textParserEngineers" :key="index"
                            class="box-item" 
                            effect="dark" 
...
import {
    ...
    saveDataPlan,
    chatRole,
} from '@/api/base/im/scene/dataAnalysis'
...
function handleGetScene() {
    getScene(currentSceneId.value).then(res => {
      ...
      if(genStatus == 0 && isCheckGenStatus.value){ // 未生成章节
        ElMessageBox.confirm( '检测到数据分析指令['+ currentSceneInfo.value.analysisPromptContent +']是否执行分析规划？', '提示', {
        ...
        .then(() => {
            updateSceneGenStatus(currentSceneId.value , 1).then(res => {
                person.value = currentSceneInfo.value.textParserEngineers[0];
                message.value = currentSceneInfo.value.analysisPromptContent;
