    ...
    >
      <template #default>
        ...
        <el-form ref="formRef" :model="formData" :rules="rules" size="large" :label-position="'top'">
          ...
          <!-- 文档分析助手 -->
          <el-form-item label="选择内容分析专员" prop="summaryAgentEngineer">
            <el-scrollbar class="scrollbar">
              <div class="custom-radio-group" >
                <div
                  v-for="engineer in summaryAgentEngineers"
                  :key="engineer.id"
                  class="custom-radio"
                  :class="{ 'selected': formData.summaryAgentEngineer === engineer.id }"
                  @click="selectSummaryAgent(engineer.id)"
                >
          ...
          <!-- 文档审核助手 -->
          <el-form-item label="选择案例查询专员" prop="caseQueryAgentEngineer">
            <el-scrollbar class="scrollbar">
              <div class="custom-radio-group" >
                <div
                  v-for="engineer in caseQueryAgentEngineers"
                  :key="engineer.id"
                  class="custom-radio"
                  :class="{ 'selected': formData.caseQueryAgentEngineer === engineer.id }"
...
import { initAgents } from '@/api/base/im/scene/documentReaderSceneInfo';
...
const formData = ref({
  summaryAgentEngineer: null,
  caseQueryAgentEngineer: null,
  contentInput: ''
});
...
const handleConfirm = async() => {
  ...
  if (valid) {
      console.log('点击了确定按钮');
      console.log('选中的大纲设计工程师:', formData.value.summaryAgentEngineer);
      console.log('选中的章节工程师:', formData.value.caseQueryAgentEngineer);
      ...
      emit('handleGetScene')
      // emit('openChatBox' , formData.value.summaryAgentEngineer , formData.value.contentInput)
...
const selectSummaryAgent = (id) => {
  formData.value.summaryAgentEngineer = id;
};

const selectChapterEngineer = (id) => {
  formData.value.caseQueryAgentEngineer = id;
};
...
//         caseQueryAgentEngineer.value = response.data;
//       }
...
/** 打开弹出窗口 */
const handleOpen = (sceneInfo , summaryAgentEngineersVal , caseQueryAgentEngineerVal) => {
  dialogVisible.value = true;
  currentSceneInfo.value = sceneInfo;
  // handleRoleBySceneIdAndAgentType();

  summaryAgentEngineers.value = summaryAgentEngineersVal ;
  caseQueryAgentEngineers.value = caseQueryAgentEngineerVal ;
};
