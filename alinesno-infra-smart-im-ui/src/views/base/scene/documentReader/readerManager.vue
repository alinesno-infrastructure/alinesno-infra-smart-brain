              ...
              <div class="channel-container-panel" style="margin-top:20px">
                <el-row>
                  <el-col :span="6" v-for="(item, index) in pagerList" :key="index" style="padding:8px;">
                    <div class="exam-pager-card-container" @click="enterDocumentParser(item)">
...
/** 进入长文本编辑界面 */
function enterDocumentParser(item) {
  const path = '/scene/documentReader/documentParser';
  router.push({
    path: path,
    query: { 
      'documentId': item.id ,
      'channelStreamId': item.id , 
