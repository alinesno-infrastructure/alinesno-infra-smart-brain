<template>
    <div class="review-page-contaier">
        ...
        <el-row :gutter="20">
            ...
            <el-col :span="19">
                <div class="review-group-add">
                    文档列表
                    <span>
                        <el-button type="warning" bg text @click="importRuleDialog">
                            <el-icon>
                                <Upload />
                            </el-icon> 导入知识库
                        </el-button>
        ...
        <!-- 导入文档弹出框 -->
        <el-dialog v-model="upload.open" title="导入文档" width="700px">
            <el-upload ref="uploadRef" :limit="1"
                accept=".xlsx,.xls,.csv,.txt,.pdf,.doc,.docx,.ppt,.pptx,.png,.jpeg,.jpg" style="width:100%"
                :headers="upload.headers" :action="upload.url + '?groupId=' + currentGroup.id + '&sceneId=' + sceneId" :disabled="upload.isUploading"
...
const handleListGroup = () => {
    ...
    listGroup(sceneId.value).then(res => {
